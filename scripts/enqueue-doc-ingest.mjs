...
import { DOCS_INGESTION_V1_JOB_NAME, docsVersionIngestionJobId } from "@ai-email/shared";
...
function fail(message) {
  console.error(JSON.stringify({ event: "doc.ingest.enqueue.error", message }));
  process.exit(1);
}
...
const jobId = docsVersionIngestionJobId({ tenantId, docId, versionId });
...
try {
  ...
  if (existing) {
    console.log(
      JSON.stringify({
        event: "doc.ingest.enqueue.reused",
        queue: "docs_ingestion",
        name: DOCS_INGESTION_V1_JOB_NAME,
        jobId
      })
    );
    process.exit(0);
